...
function wrapImageWithLightBox() {
  ...
  lightbox.addEventListener("click", (e) => {
    const excludeTarget = [img, prevButton, nextButton, prevButton.querySelector("i"), nextButton.querySelector("i")];
...
function activeArticleToc() {
  ...
  const activeLi = document.querySelector("#tree li.file.active, #tree li.directory .directory.active");
  ...
  window.addEventListener("scroll", function () {
    ...
    anchorList.forEach((anchor) => {
      const tocLinkList = document.querySelectorAll(`.article-toc a[href="${anchor.getAttribute("href")}"]`);
      ...
      if (title.length) {
        ...
        if (treeNodes.length) {
          ...
          if (treeNodes.length > 1) {
            const categories = document.querySelector("#article-categories span:last-child a")?.innerHTML.trim();
            if (categories) {
              const directoryList = document.querySelectorAll("#tree li.directory a:contains('" + categories + "')");
...
function setupNavigation() {
  document.addEventListener("click", function (e) {
    ...
    if (
      target &&
      target.matches("#menu a:not(.item-icon), #index a, #tree li.file > a[href], #tree li.directory > a[href], .post-guide a")
    ) {
      e.preventDefault();
  ...
  searchInput.addEventListener("input", function (e) {
    ...
    if (searchResultNodes.length) {
      searchResultNodes.forEach((result) => {
        ...
        const liNodeList = document.querySelectorAll(`#tree li.directory:has(a[title="${linkTitle}"])`);
